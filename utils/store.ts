...
export const useStore = create<StoreState>((set, get) => ({
  ...
  get filteredVehicles() {
    ...
    return state.vehicles.filter(vehicle => {
      if (state.filters.type && vehicle.type !== state.filters.type) return false;
      if (state.filters.minPrice && vehicle.price < state.filters.minPrice) return false;
      if (state.filters.maxPrice && vehicle.price > state.filters.maxPrice) return false;
      if (state.filters.location && vehicle.location !== state.filters.location) return false;
      if (state.filters.showAvailableOnly && !vehicle.available) return false;
      return true;
    });
  }
}));